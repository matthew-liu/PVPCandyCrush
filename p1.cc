...
// network & parallel code //
hw5_net::ClientSocket *peerSocket_ptr;
atomic<int> updated (0);

void thread_read() {
  while(true) {
    string move = Module::readMessage(*peerSocket_ptr);
    if (move == "") {
...
void sendMove(int direction) {
    ...
    free(dumped);
    json_decref(move);
    (*peerSocket_ptr).WrappedWrite(s.c_str(), s.length());
}
//*************************//
...
static void draw (GtkWidget *window) {
  ...
  if ((*gamestate).currentscore >= maxScore || (*p2_gamestate).currentscore >= maxScore) {
    (*peerSocket_ptr).WrappedWrite("{end}", 5);
    char str[120];
...
static void set_up_p2(char* filename) {
  // send maxScore to p2
  json_t * score = json_object();

  Utility::json_integer_set(score, "maxScore", maxScore);

  char* dumped = json_dumps(score, JSON_ENCODE_ANY);
  string s(dumped);
  free(dumped);
  json_decref(score);
  (*peerSocket_ptr).WrappedWrite(s.c_str(), s.length());

  // send input json_file to p2
  GameInstance *temp = new GameInstance();

  Utility::constructFromFile(filename, temp->gamedef, temp->gamestate);

  char* result;
  Utility::serialize(&result, *temp);
  string content(result);

  free(result);
  delete(temp);

  (*peerSocket_ptr).WrappedWrite(content.c_str(), content.length());
}

int main (int argc, char **argv) {

  if (argc != 1 && argc != 2 && argc != 3) {
      cout << "Usage: " << argv[0] << " [json file] [maxScore]" << endl;
      cout << "  json file is by default test25by5.json, " << endl
           << "  maxScore  is by default 20" << endl;
      exit(1);
  }
  char* inputFile;
  ...
  if (argc == 3) {
    ...
    inputFile = "test25by5.json";
    maxScore = 20;
    if (argc == 2) maxScore = atoi(argv[1]);
  }
  ...
  Utility::constructFromFile(inputFile, *gamedef, *gamestate);
  Manipulate::update(*gamedef, *gamestate);

  Utility::constructFromFile(inputFile, *p2_gamedef, *p2_gamestate);
  Manipulate::update(*p2_gamedef, *p2_gamestate);
  ...
  try {
    int socketFd;
    hw5_net::ServerSocket sock(33360);
    ...
    cout << "P2 Connected from: " << clientDNSName << ":" << clientPort << endl;
